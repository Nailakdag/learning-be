...
router.patch("/update", async (req, res) => {
    ...
    try {
        ...
        if (body.permissions && Array.isArray(body.permissions) && body.permissions.length > 0) {
            ...
            let removedPermissions = permissions.filter((item) => !body.permissions.includes(item.permission));
            let newPermissions = body.permissions.filter((item) => !permissions.map((p) => p.permission).includes(item));
